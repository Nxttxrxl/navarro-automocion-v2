  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const stdModel = (formData.modelo || '')
          .toUpperCase()
          .replace(/[^A-Z0-9]/g, '_');
        const stdYear = formData.year || '0000';

        let baseName = `${stdBrand}_${stdModel}_${stdYear}`
          .replace(/__+/g, '_')
          .replace(/_$/, '');
      ...
      const carData = {
        ...
        transmision: formData.transmision || 'Manual',
      ...
      if (editingCar) {
        ...
        if (updateError) {
          throw new Error(
            `Error al actualizar en la base de datos: ${updateError.message}`,
          );
        }
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white border border-slate-200 rounded-2xl shadow-xl p-8 w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            {message.text && (
              <div
                className={`p-4 rounded-lg text-sm font-medium ${message.type === 'error' ? 'bg-red-50 border border-red-200 text-red-600' : 'bg-green-50 border border-green-200 text-green-600'}`}
              >
                {message.text}
              </div>
            )}
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white border border-slate-200 rounded-2xl shadow-md p-8">
          ...
          {message.text && (
            <div
              className={`mb-6 p-4 rounded-lg flex items-center gap-3 ${message.type === 'error' ? 'bg-red-50 border border-red-200 text-red-600' : 'bg-green-50 border border-green-200 text-green-600'}`}
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  className={`w-full px-4 py-2.5 bg-white border rounded-lg text-slate-900 focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all outline-none font-medium ${
                    formData.etiqueta === '0'
                      ? 'border-blue-500'
                      : formData.etiqueta === 'ECO'
                        ? 'border-green-600'
                        : formData.etiqueta === 'C'
                          ? 'border-green-500'
                          : formData.etiqueta === 'B'
                ...
                >
                  ...
                  <option value="ECO" className="text-green-700 font-bold">
                    ECO
                  </option>
                  <option value="C" className="text-green-600 font-bold">
                    C - Verde
                  </option>
                  <option value="B" className="text-yellow-600 font-bold">
                    B - Amarillo
                  </option>
                  <option value="Sin etiqueta" className="text-slate-400">
                    Sin etiqueta
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  <option value="Activo" className="text-green-600 font-bold">
                    Activo (En Stock)
                  </option>
                  <option
                    value="Reservado"
                  ...
                  <option value="Vendido" className="text-red-600 font-bold">
                    Vendido
                  </option>
                </select>
              </div>
              ...
              >
                {loading ? (
                  <>
                    ...
                    <span>
                      {editingCar ? 'Actualizando...' : 'Guardando...'}
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span>
                      {editingCar ? 'Actualizar Vehículo' : 'Guardar Vehículo'}
                    </span>
                  </>
                )}
              </button>
          ...
          <div className="mt-12 border-t border-slate-200 pt-8">
            ...
            {loadingCars ? (
              ...
              <div className="text-center py-12 bg-slate-50 rounded-lg border border-slate-200">
                <span className="material-symbols-outlined text-slate-300 text-6xl mb-3">
                  inventory_2
                </span>
